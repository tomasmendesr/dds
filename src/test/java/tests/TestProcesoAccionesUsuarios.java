...
public class TestProcesoAccionesUsuarios {
	...
	// Sin criterio, se afectan a todas las terminales
	sinCriterio = new SinCriterio();
	
	}
	
	// Proceso1: Agrego acciones a las terminales segun la comuna
	@Test
	public void agregoObserverReportesTotalesALaTerminalAbasto(){ 
		...
		agregarAccionUsuario.realizarAccion();
	...
	@Test
	public void laCantidadDeAfectadosEs1(){
		agregarAccionUsuario= new AccionesUsuarios(segunComuna, "agregar", observerReportesTotales);
		agregarAccionUsuario.agregarTerminal(terminalAbasto);
		agregarAccionUsuario.agregarTerminal(terminalFlorida);
		resultadoProceso = agregarAccionUsuario.realizarAccion();
		Assert.assertEquals(1, resultadoProceso.getCantElementosAfectados()) ;
	}
	
	
	// Proceso2: agrego acciones a todas las terminales
	@Test
	public void agregoElObserverALasTerminales(){
		agregarAccionUsuario = new AccionesUsuarios(sinCriterio, "agregar", observerReportesTotales);
		agregarAccionUsuario.agregarTerminal(terminalAbasto);
		agregarAccionUsuario.agregarTerminal(terminalFlorida);
		agregarAccionUsuario.realizarAccion();
		Assert.assertEquals(1,terminalAbasto.getObservers().size());
